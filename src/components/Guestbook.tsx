...
function StickyNote({
  ...
  return (
    <article
      className={`sticky-note ${stickyColors[color]} w-full px-10 pt-10 pb-5 transition-transform duration-300 hover:rotate-1`}
      style={{ transform: `rotate(${rotation}deg)` }}
    >
      <p className="font-display whitespace-pre-line text-[1.05rem] leading-snug text-espresso md:text-lg">
...
function ReceiptNote({
  ...
  return (
    <article
      className="receipt w-full rounded-sm px-6 py-5 transition-transform duration-300 hover:rotate-1"
      style={{ transform: `rotate(${rotation}deg)` }}
    >
      <p className="mt-1 font-display text-sm text-sage">{note.author}</p>
      <p className="mt-4 whitespace-pre-line text-md leading-relaxed text-espresso">
        {note.text}
      </p>
      <p className="mt-5 border-t border-dashed border-latte pt-3 text-center text-lg text-espresso-muted">
        ♥
      </p>
    </article>
  );
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <Reveal delay={80}>
          <div className="guestbook-board mt-12 p-6 md:p-10">
            <ul className="grid grid-cols-1 items-start gap-6 sm:grid-cols-2 sm:gap-8">
